...
namespace TerraCraft.Core.DataStructures.GridCrafting
{
    public class RecipeDatabase
    {
        ...
        // 根据输入物品类型获取所有配方（只要输入中包含该物品）
        public List<GriddedRecipe> GetRecipesByInput(int itemType)
        {
            if (!_cacheInitialized) InitializeCache();

            return Recipes
                .Where(r => r.Ingredients != null && r.Ingredients.Any(i =>
                    i.ItemType == itemType ||
                    (!string.IsNullOrEmpty(i.RecipeGroup) && RecipeGroupResolver.ItemMatchesGroup(itemType, i.RecipeGroup))
                ))
                .ToList();
        }

        // 根据输出物品类型获取所有配方
        public List<GriddedRecipe> GetRecipesByOutput(int itemType)
        {
            if (!_cacheInitialized) InitializeCache();
            
            return Recipes.Where(r => r.Outputs != null && r.Outputs.Any(o => o.ItemType == itemType)).ToList();
        }
